		...
		if(urls[0].equals("") || urls[0].equals("index") && urls.length == 1) {
			...
			if (context.equals("/home/")) {
				...
				if(request.getSession().getAttribute("id") == null) {
					...
					if (urls.length == 1) {
						System.out.println("urls.length == 1");
...
//						System.out.println(pageboard.getList());
						...
						BoardDAO dao = new BoardDAO();
						dao.insert(board);
						PageBoard pageboard = dao.pagemove(1,0);
						request.setAttribute("pageboard", pageboard);
						page = "list.jsp";
		...
		System.out.println("보내기 직전"+context+page);
